...
import { PairVoicePanel } from './webviewPanel';
import { collectContext, initTerminalCapture, navigateToFile, applyEdit } from './contextCollector';
import { VoiceServer } from './voiceServer';

const HEALTH_CHECK_INTERVAL = 30_000;
const SETUP_COMPLETE_KEY = 'pairvoice.setupComplete';

let _panel: PairVoicePanel | undefined;
let _statusBar: vscode.StatusBarItem | undefined;
let _isListening = false;
let _voiceServer: VoiceServer | undefined;
let _lastVapiStartPayload: any | undefined;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  _voiceServer = new VoiceServer(context.extensionUri.fsPath, (event) => {
    ...
    if (event.type === 'transcript') {
      ...
      void navigateToFile(event.filePath, event.line || 0);
      if (event.speech) {
        _panel?.postMessage({ type: 'response', text: event.speech });
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.openPanel', () => {
      _panel = PairVoicePanel.createOrShow(context.extensionUri);
      _panel.onMessage((msg: any) => handlePanelMessage(msg, context));
      void refreshIndexStatus(context);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.startVoice', async () => {
      if (!_panel) {
        _panel = PairVoicePanel.createOrShow(context.extensionUri);
        _panel.onMessage((msg: any) => handlePanelMessage(msg, context));
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.indexWorkspace', async () => await indexWorkspace())
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.attachDocument', async () => await attachDocument())
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.configureVapi', async () => await requestVapiKeys(context))
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.storeGithubToken', async () => await promptForGithubToken(context))
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('pairvoice.checkBackend', async () => await refreshBackendStatus(context))
  );
...
async function startListening(context: vscode.ExtensionContext) {
  ...
  const vapiKey = config.get<string>('vapiPublicKey', '');
  const assistantId = config.get<string>('vapiAssistantId', '');
  const backendUrl = config.get<string>('backendUrl', 'http://localhost:8000');

  if (!vapiKey || !assistantId) {
    vscode.window.showErrorMessage(
      'PairVoice: Please configure your Vapi public key and assistant ID (PairVoice settings).'
    );
    return;
  }
...
async function handlePanelMessage(msg: any, context: vscode.ExtensionContext) {
  ...
  switch (msg.type) {
    ...
    case 'stopVoice':
      ...
      break;
    case 'navigate':
      await navigateToFile(msg.filePath, msg.line || 0);
      break;
    case 'applyFix':
      if (msg.filePath && msg.newContent) {
        const ok = await applyEdit(msg.filePath, msg.newContent);
        _panel?.postMessage({
 ...
 */
async function handleDocumentSave(doc: vscode.TextDocument, context: vscode.ExtensionContext) {
  ...
  const result = (await postJson(`${backendUrl}/index/file-update`, payload, { showError: false })) as { chunks_updated?: number };
...
async function indexWorkspace() {
  ...
  const backendUrl = vscode.workspace.getConfiguration('pairvoice').get<string>('backendUrl', 'http://localhost:8000');
  await vscode.window.withProgress({
    ...
    const codeChunks = Number((result as any)?.code_chunks ?? 0);
    const docChunks = Number((result as any)?.doc_chunks ?? 0);
 ...
 */
async function refreshBackendStatus(context: vscode.ExtensionContext) {
...
async function refreshIndexStatus(context: vscode.ExtensionContext) {
  const folder = vscode.workspace.workspaceFolders?.[0];
  if (!folder) return;
  const backendUrl = vscode.workspace.getConfiguration('pairvoice').get<string>('backendUrl', 'http://localhost:8000');
  await refreshIndexStatusFromFolder(folder, backendUrl);
...
async function refreshIndexStatusFromFolder(folder: vscode.WorkspaceFolder, backendUrl: string) {
  ...
  try {
    ...
    const data = (await response.json()) as { code_chunks?: number; doc_chunks?: number };
 ...
 */
async function requestVapiKeys(context: vscode.ExtensionContext) {
  const config = vscode.workspace.getConfiguration('pairvoice');
  const currentKey = config.get<string>('vapiPublicKey', '');
  const currentAssistant = config.get<string>('vapiAssistantId', '');
  const alreadyConfigured = !!currentKey && !!currentAssistant;
  const wizardDone = context.globalState.get<boolean>(SETUP_COMPLETE_KEY, false);

  if (alreadyConfigured && wizardDone) return;

  const updated: { key?: string; assistant?: string } = {};
 ...
 */
async function promptForGithubToken(context: vscode.ExtensionContext) {
 ...
 */
async function postJson(url: string, data: object, options: { showError?: boolean } = {}) {
  try {
    const response = await fetch(url, {
      method: 'POST',
